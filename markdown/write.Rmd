...
lab <- "\\caption{\\label{tab:demos} Demographics of Neighborhoods}"
...
j <- bind_rows(j, data.frame(V1 = c(insert1), n = c(7.5))) %>% 
  arrange(n) %>% 
  select(-n)
...
```{r marg1, echo = FALSE, message = F, warning = F, fig.width = 5, fig.height = 5, fig.cap = "\\label{fig:marg1}Marginal Effect of Formerly Incarcerated Residents on Precinct Support for Amendment 4", fig.pos = "H", fig.align = "center"}
j <- readRDS("./temp/marg_support_am4.rds")
print(j)
```
