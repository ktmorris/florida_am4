match_rolls_to_doc <- function(doc_data, doc_id,
  ...
  doc_data <- doc_data %>%
    select(all_of(doc_id), first_name, last_name, middle_name, dob) %>%
    mutate_at(vars(first_name, middle_name, last_name), ~tolower(gsub("[[:punct:]]| ", "", .))) %>%
  ...
  roll_data <- roll_data %>%
    select(all_of(roll_id), first_name, last_name, middle_name, dob) %>%
    mutate_at(vars(first_name, middle_name, last_name), ~tolower(gsub("[[:punct:]]| ", "", .))) %>%
    group_by(first_name, middle_name, last_name, dob) %>%
    mutate(count = row_number()) %>%
    ungroup()
  ...
  doc_rem <- anti_join(doc_data, match1, by = c(all_of(doc_id))) %>%
  ...
  roll_rem <- anti_join(roll_data, match1, by = c(all_of(roll_id))) %>%
    mutate(middle_in = substr(middle_name, 1, 1),
           group = middle_in != middle_name) %>%
  ...
  doc_rem <- anti_join(doc_rem, match2, by = c(all_of(doc_id))) %>%
  ...
  roll_rem <- anti_join(roll_rem, match2, by = c(all_of(roll_id))) %>%
    mutate(group = !is.na(middle_in)) %>%
    select(-middle_in) %>%
    group_by(first_name, last_name, group, dob) %>%
    mutate(count = row_number()) %>%
  ...
  match <- bind_rows(match1, match2, match3) %>%
    ...
    select(all_of(doc_id), all_of(roll_id), run)

  doc_rem <- anti_join(doc_data, match, by = c(all_of(doc_id))) %>%
    select(all_of(doc_id))
  roll_rem <- anti_join(roll_data, match, by = c(all_of(roll_id))) %>%
    select(all_of(roll_id))

  big_doc <- full_join(big_doc, match, by = c(all_of(doc_id))) %>%
    select(-dob) %>%
    rename_at(vars(ends_with("_name")), ~ paste0(., "_doc"))

  merge <- full_join(big_doc, big_roll, by = c(all_of(roll_id))) %>%
